...
// input vectors. The header says 1..Nlins, but this avoids errors if the caller passed a 0-based vector.
...
// Experiment:   ClosestOscFree (E2)
// Flag:         --closest-osc-free
// Origin:       SAIL comment
// Source:       Parshl-source.txt L1250–L1317, GetClosestFrq; gap G10
//               (docs/V2_OSCILLATOR_LIFECYCLE_AUDIT.md)
// Verbatim:     GetClosestFrq contains a path for a nearest free oscillator
//               that is not wired into the V2 dispatch.
// Summary:      Replace first-dead-slot selection with closest-PrvOscFrq
//               selection among dead slots during the Add phase.
// Expected:     Reduced frequency glide at oscillator birth; lower M5
//               (frequency error at new-partial birth frames).
// Metrics:      M2, M3, M5
// Contract:     Experimental only.  Disabled by default.
//               V2 baseline is bit-identical when closest_osc_free == false.
std::int64_t ParshlTracker::find_closest_free_osc(double target_frq) const noexcept {
  std::int64_t best_osc  = 0;
  ...
  for (std::int64_t osc = 1; osc <= st_.Noscs; ++osc) {
    ...
    if (st_.LinOfOsc[oi] <= 0
        && st_.OscAmp[oi]    == 0.0
        && st_.PrvOscAmp[oi] == 0.0) {
      const double prv  = st_.PrvOscFrq[oi];
      // Slots with no frequency history (PrvOscFrq==0) get +inf distance so they
      // lose to any slot with real history; ties broken by slot index (lower wins).
      const double dist = (prv > 0.0)
          ? std::abs(prv - target_frq)
          : std::numeric_limits<double>::infinity();
      if (dist < best_dist) { best_dist = dist; best_osc = osc; }
    }
  }
  // Fallback: all dead slots have no history (all +inf) → same result as baseline.
  if (best_osc == 0) return find_free_osc();
  return best_osc;
}
// end [V3 EXPERIMENT E2 --closest-osc-free]
...
void ParshlTracker::update_map(std::int64_t Frame1,
  ...
  if (Nlins == 0) {
    ...
    if (p_.legacy_mode) {
      ...
      while (true) {
        ...
        // Contract:     Experimental only.  Disabled by default.
        //               V2 baseline is bit-identical when closest_osc_free == false.
        const std::int64_t free_slot = p_.closest_osc_free
            ? find_closest_free_osc(get_lin(LinFrq, CurLin, Nlins))
            : find_free_osc();
        // Metric: count recycled slots where closest-free differed from first-free.
        if (p_.closest_osc_free && free_slot > 0 && free_slot != find_free_osc())
          ++ts_.cof_reuse_count;
        // end [V3 EXPERIMENT E2 --closest-osc-free]
        if (free_slot > 0) {
          CurOsc   = free_slot;
  ...
  for (std::int64_t CurOsc = 1; CurOsc <= p_.MaxOscs; ++CurOsc) {
    ...
    // InstantRise does NOT appear in OutAF in SAIL; only in the Frame 1
    // initialisation (L1481), a completely different context. The prior guard
    // on p_.InstantRise was factually incorrect (audit C-1).
