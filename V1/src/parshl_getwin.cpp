...
[[nodiscard]] static std::vector<double> gen_basic(int Wtype, int L, double P3, double P4) {
  ...
  switch (Wtype) {
    ...
    case Kaiser: {
      // NOTE: SAIL passed beta directly. This port takes attenuation
      // in dB and converts via Kaiser's formula. If replicating SAIL
      // exactly, pass beta converted to dB: dB = 2.285*(Nw-1)*pi*df + 7.95
...
void GetWin(sail::Array1D<double>& W, int Wtype, int Nw, double P3, double P4) {
  if (Nw % 2 == 0) {
    // SAIL used odd-length windows exclusively for zero-phase.
    // Even Nw breaks the FFT buffer layout in parshl_stft.cpp.
