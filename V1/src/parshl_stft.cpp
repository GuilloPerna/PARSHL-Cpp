...
void StftFrameRunner::compute_db(const std::vector<double>& x, long pos, sail::Array1D<double>& Xdb) {
  ...
  // Zeros cover the middle zero-padding region; the two loops below only touch
  // the non-zero edges, so no separate clear is needed for those regions.
  for (int n = 0; n < Nfft; ++n) {
    fft.in[static_cast<size_t>(n)] = 0.0;
  }

  // SAIL L1698/L1701: Sandi (read) + window multiplication X[i] <- X[i]*WinBuf[i],
  // placed into the FFT buffer with the SAIL zero-phase (ifftshift) layout:
  //
  //   buf[0..Mh]           ← windowed[Mh..Nx-1]  (center + positive-time half)
  //   buf[Mh+1..Nfft-Mh-1] = 0                   (zero-padding in the MIDDLE)
  //   buf[Nfft-Mh..Nfft-1] ← windowed[0..Mh-1]   (negative-time half, wrapped)
  //
  // SAIL equivalence (1-based, M=Nx, N=Nfft):
  //   positions 1..(M+1)/2   ← samples 1..(M+1)/2
  //   positions N-(M-1)/2..N ← samples (M+3)/2..M
  //
  // This makes the spectrum zero-phase: for symmetric windows the output is
  // real-valued, and phase estimates for partials are correctly referenced to
  // the centre of the analysis frame (pos+Mh), not its left edge.
  const int Mh = (Nx - 1) / 2;  // Nx must be odd (CRITICAL-6); Mh = (M-1)/2

  // Center + positive-time half → buf[0..Mh]
  for (int n = 0; n <= Mh; ++n) {
    long idx = pos + Mh + n;
    double s = 0.0;
    if (idx >= 0 && idx < static_cast<long>(x.size())) s = x[static_cast<size_t>(idx)];
    fft.in[static_cast<size_t>(n)] = s * win[static_cast<size_t>(Mh + n)];
  }

  // Negative-time half → buf[Nfft-Mh..Nfft-1]
  for (int n = 0; n < Mh; ++n) {
    long idx = pos + n;
    double s = 0.0;
    if (idx >= 0 && idx < static_cast<long>(x.size())) s = x[static_cast<size_t>(idx)];
    fft.in[static_cast<size_t>(Nfft - Mh + n)] = s * win[static_cast<size_t>(n)];
  }
