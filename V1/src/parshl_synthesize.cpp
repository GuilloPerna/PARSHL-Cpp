...
void parshl_synthesize_additive(
  ...
  bool oscphs_double             // [V3 E4] if true: preserve fractional phase
) {
  ...
  auto synth_one_osc = [&](int Zosc) {
    ...
    double Inc = ss.Mag * PrvOscFrq[static_cast<std::size_t>(Zosc)];
    const double Dinc = ss.Mag * (OscFrq[static_cast<std::size_t>(Zosc)] - PrvOscFrq[static_cast<std::size_t>(Zosc)]) / static_cast<double>(Nhop);
    double Phs = ss.OscPhs[static_cast<std::size_t>(Zosc)];
    ...
    // SAIL L1664-1666: Phs <- IF Phs>=SinSiz THEN Phs-SinSiz ELSE IF Phs<0 THEN Phs+SinSiz
    // SAIL wraps per-sample inside the loop (safe there because Inc<SinSiz for freq<Fs).
    // C++ wraps once per hop: after Nhop samples Phs grew by Nhop*Inc >> SinSiz for any
    // audible frequency (fails above freq > Fs/Nhop ≈ 172 Hz at Fs=44100, Nhop=256).
    // FIX: replace single-step subtraction with std::fmod — equivalent to SAIL's per-sample
    // wrap for the stored OscPhs value, frequency-independent, eliminates int overflow risk.
    Phs = std::fmod(Phs, static_cast<double>(ParshlSynthState::SinSiz));
    if (Phs < 0.0) Phs += static_cast<double>(ParshlSynthState::SinSiz);
    // SAIL: OscPhs is INTEGER ARRAY; assigning REAL->INTEGER truncates toward
    // zero (Parshl-source.txt L1046, L1667). The fractional part of Phs is
    // discarded at every hop boundary. Reproduce that here (audit C-2).
    // [V3 E4] --oscphs-double: skip the integer truncation to preserve full
    // fractional phase across hop boundaries. Origin Type C: numerical precision.
    // Source: Parshl-source.txt L1046/L1667 INTEGER ARRAY (audit C-2).
    // Contract: experimental; flag off = SAIL-faithful integer truncation (V2 baseline).
    if (oscphs_double) {
      ss.OscPhs[static_cast<std::size_t>(Zosc)] = Phs;
    } else {
      ss.OscPhs[static_cast<std::size_t>(Zosc)] = static_cast<double>(static_cast<int>(Phs));
    }
    // end [V3 E4]
  };
