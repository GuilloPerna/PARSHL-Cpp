...
void ParshlTracker::update_map(std::int64_t Frame1,
  ...
  for (std::int64_t CurOsc = 1; CurOsc <= p_.MaxOscs; ++CurOsc) {
    ...
    // InstantRise does NOT appear in OutAF in SAIL; it only appears in the
    // Frame 1 initialisation (L1481), a completely different context. The
    // previous guard on p_.InstantRise was factually incorrect (audit C-1).
