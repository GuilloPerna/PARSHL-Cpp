...
//
// Reads a WAV, runs some frames and displays:
//
...
int main(int argc, char** argv) {
  ...
  // [V3 E3-B] --amp-refine-max-db=N: clamp the per-oscillator amplitude correction
  // to at most N dB. 0.0 = no clamp (original E3 behaviour).
  double amp_refine_max_db = 0.0;
  // [V3 E3-C] --dbspec-amp-only: amplitude refinement from Xdb WITHOUT frequency refinement.
  // Decouples set_osc_amp from set_osc_frq. Hypothesis: amplitude correction alone is neutral
  // in tracker (stop_count/dfmax unchanged); rescues E3 as a pure amplitude fix.
  bool dbspec_amp_only = false;
  ...
  // [V3 E5-B] --dbspec-match-alpha=N: hybrid score weight (dB/Hz).
  // score(b) = Xdb[b] - alpha * |frq(b) - predicted_frq|
  // 0.0 = energy-only (E5 original); >0 penalises bins far from predicted_frq.
  double dbspec_match_alpha = 0.0;
  // [V3 E5-C] --dbspec-match-factor=F: conservative override criterion.
  // Override activates only if dist_dbspec < dist_linfrq * F.
  // F=1.0 = E5 original (any bin strictly closer overrides).
  // F<1.0 = DBspec bin must be substantially closer to justify override.
  double dbspec_match_factor = 1.0;
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    // [V3 E3-C] Amplitude-only refinement: Xdb peak search + set_osc_amp, no set_osc_frq.
    if (a == "--dbspec-amp-only") {
      dbspec_amp_only = true;
      continue;
    }
    // [V3 E3-B] Amplitude correction clamp: limit per-oscillator amp change to N dB.
    if (a.rfind("--amp-refine-max-db=", 0) == 0) {
      amp_refine_max_db = std::stod(a.substr(std::string("--amp-refine-max-db=").size()));
      continue;
    }
    ...
    // [V3 E5-B] Hybrid score weight: penalise bins far from predicted frequency.
    if (a.rfind("--dbspec-match-alpha=", 0) == 0) {
      dbspec_match_alpha = std::stod(a.substr(std::string("--dbspec-match-alpha=").size()));
      continue;
    }
    // [V3 E5-C] Conservative override threshold.
    if (a.rfind("--dbspec-match-factor=", 0) == 0) {
      dbspec_match_factor = std::stod(a.substr(std::string("--dbspec-match-factor=").size()));
      continue;
    }
  ...
  // [V3 E3-C] --dbspec-amp-only is independent of --dbspec-refine: it activates the Xdb
  // search block but skips set_osc_frq. If both are active, freq refinement also runs.
  if (dbspec_amp_only && dbspec_refine) {
    std::cerr << "[V3 E3-C] warning: --dbspec-amp-only with --dbspec-refine active: "
                 "frequency correction WILL ALSO run.\n";
  }
  ...
  // These flags only prevent the driver from hard-coding values for tracker exploration;
  ...
  for (std::int64_t f = 0; f < maxFrames; ++f) {
    ...
    if (dbspec_match && !reverse_analysis) {
      ...
      for (std::int64_t osc = 1; osc <= lim; ++osc) {
        ...
        // Bin search: max of hybrid score = Xdb[b] - alpha * |frq(b) - predicted_frq|.
        // [V3 E5-B] alpha=0 → pure energy (E5 original); alpha>0 → penalise distance.
        long best_bin = (bin_c >= 1 && bin_c <= (long)Nspec) ? bin_c : lo;
        {
          const double frq_b0 = static_cast<double>(fs)
              * (static_cast<double>(best_bin) - 1.0) / static_cast<double>(Nfft);
          double best_score = Xdb[best_bin]
              - dbspec_match_alpha * std::fabs(frq_b0 - predicted_frq);
          for (long b = lo; b <= hi; ++b) {
            const double frq_b = static_cast<double>(fs)
                * (static_cast<double>(b) - 1.0) / static_cast<double>(Nfft);
            const double score = Xdb[b]
                - dbspec_match_alpha * std::fabs(frq_b - predicted_frq);
        ...
        if (dist_dbspec < dist_linfrq * dbspec_match_factor) {
    ...
    if ((dbspec_refine || dbspec_amp_only) && !reverse_analysis) {
      ...
      for (std::int64_t osc = 1; osc <= lim; ++osc) {
        ...
        // [V3 E3-C] --dbspec-amp-only: skip frequency update, amplitude-only path.
        if (!dbspec_amp_only) {
        ...
        // Contract:     Experimental. Disabled by default. V2 baseline unchanged when off.
        if (dbspec_refine_amp || dbspec_amp_only) {
          ...
          // [V3 E3-B] --amp-refine-max-db clamp: limit correction to N dB in either direction.
          // When amp_before == 0 (new oscillator), clamp against SigScl as reference
          // to avoid log10(x/0) = inf bypassing the gate.
          double amp_to_set = amp_refined;
          if (amp_refine_max_db > 0.0) {
            const double ref   = (amp_before > 0.0) ? amp_before : SigScl;
            const double ratio = std::pow(10.0, amp_refine_max_db / 20.0);
            amp_to_set = std::min(amp_refined, ref * ratio);
            amp_to_set = std::max(amp_to_set,  ref / ratio);
          }
          tracker.set_osc_amp(osc, amp_to_set);
          dbspec_amp_deltas.push_back(std::abs(amp_to_set - amp_before));
        }
  ...
  if (out_enabled) {
    ...
    // [FIX] Normalise from SAIL units to [-1.0, 1.0] before writing the synthesis WAV.
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << " dbspec_amp_only="          << (dbspec_amp_only   ? 1 : 0)
            << " amp_refine_max_db="        << amp_refine_max_db
            ...
            << " dbspec_match_alpha="       << std::setprecision(6) << dbspec_match_alpha
            << " dbspec_match_factor="      << std::setprecision(6) << dbspec_match_factor
